...
SELECT * FROM orders
WHERE created at BETWEEN '2025-03-01' AND '2025-03-31';
-- 9. `orders` 테이블에서 2025년 3월 12일 오전에 주문된 내역만 조회하세요.
SELECT * FROM orders
WHERE created at BETWEEN '2025-03-12 00:00:00' AND '2025-03-12 11:59:59;'
-- 10. `pizza_types` 테이블에서 이름에 'Cheese' 혹은 'Chicken'이 포함된 피자 종류를 조회하세요. (대소문자를 구분합니다)
SELECT * FROM pizza_types
WHERE name LIKE '%Cheeswe%' OR name LIKE '%Chicken%';
-- # 중급 문제
...
SELECT DATE (CREATEd_AT) AS order_date,
         COUNT(*) AS order_count,
            ...
            FROM orders
            JOIN order_details ON order_id = order_id
