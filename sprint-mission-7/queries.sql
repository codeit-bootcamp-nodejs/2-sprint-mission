...
SELECT products.*
FROM products
    JOIN product_likes ON products.id = product_likes.product_id
WHERE product_likes.user_id = 1
ORDER BY product_likes.created_at DESC
LIMIT 10 OFFSET 20
;
...
SELECT 
    products.*,
    COUNT(product_likes.id) AS like_count
FROM products
    LEFT JOIN product_likes ON products.id = product_likes.product_id
...
WHERE product_id = 1 AND create_at < '2025-03-25'
